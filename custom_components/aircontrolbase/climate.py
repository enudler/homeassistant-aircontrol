...
async def async_setup_entry(
    ...
    async_add_entities(
        AirControlBaseClimate(api, coordinator, device, hass.data[DOMAIN][config_entry.entry_id]["refresh_delay"])
...
class AirControlBaseClimate(CoordinatorEntity, ClimateEntity):
    ...
    def _should_skip_coordinator_update(self) -> bool:
        """Check if we should skip coordinator updates due to recent commands."""
        # Skip updates for refresh_delay + 20 seconds after sending a command
        return (time.time() - self._last_command_time) < (self._refresh_delay + 20)

    @property
    def _device(self) -> Dict[str, Any]:
        """Return the latest device data from the coordinator, but preserve local changes if recent command."""
        ...
        # If we recently sent a command, preserve our local state instead of using coordinator data
        if hasattr(self, '_local_device_state') and self._should_skip_coordinator_update():
            _LOGGER.debug("Skipping coordinator update due to recent command for device %s", device_id)
            return self._local_device_state
    ...
    async def async_set_temperature(self, **kwargs: Any) -> None:
        ...
        try:
            # Track command time and store local state
            self._last_command_time = time.time()
            self._local_device_state = self._device.copy()
            self._local_device_state["setTemp"] = int(temperature)
            self._local_device_state["power"] = "y"  # Device is now on
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode) -> None:
        ...
        try:
            ...
            self._last_command_time = time.time()
            self._local_device_state = self._device.copy()
            self._local_device_state["mode"] = mode_map[hvac_mode]
            self._local_device_state["power"] = operation_data["power"]
            if "wind" in operation_data:
                self._local_device_state["wind"] = operation_data["wind"]
    ...
    async def async_set_fan_mode(self, fan_mode: str) -> None:
        ...
        try:
            ...
            self._last_command_time = time.time()
            self._local_device_state = self._device.copy()
            self._local_device_state["wind"] = api_mode
